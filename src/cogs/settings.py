...
class SettingsCommands(commands.Cog):
	...
	async def setlanguage(
		...
		locale: Option(
			str,
			choices=[
				OptionChoice(_n, _l) for _l, _n in EXISTS_LOCALE_LIST.items()
			...
			if not (await GuildConfigManager.update(ctx.guild.id, gc)):
				...
				await ctx.send_followup(
					embed=embeds.Notification.error(
						description=_("CmdMsg_FailedToUpdateConfig")
					)
				)
				return
	...
	async def setindicator(
		...
		try:
			...
			await ctx.send_followup(
				embed=embeds.Notification.success(
					description=_("Cmd_setindicator_Success", _(str(enable)))
				)
			)
		except Exception:
			# 設定をリセット
	...
	async def setnotification(
		...
		auto_delete: Option(
			int, required=False, default=10, min_value=0, max_value=600
		),  # pyright: ignore[reportInvalidTypeForm]
	) -> None:
		await ctx.defer(ephemeral=True)
		...
		try:
			...
			if enable:
				...
				if not ch:
					await ctx.send_followup(
						embed=embeds.Notification.error(
							description=_("CmdMsg_TextChannelNotFound")
						)
					)
					return
				...
				if not ch.can_send():
					await ctx.send_followup(
						embed=embeds.Notification.error(
							description=_("CmdMsg_DontHavePermission_SendMessage")
						)
					)
					return
				...
				if role:
					...
					if not role.mentionable:
						await ctx.send_followup(
							embed=embeds.Notification.error(
								description=_(
									"Cmd_setnotification_RoleIsNotMentionable"
								)
							)
						)
						return
			...
			# 設定完了メッセージを送信する
			success_embed = embeds.Notification.success(
				description=_("Cmd_setnotification_Success", _(str(enable)))
			)
			if enable:
				# テキストチャンネルの項目
				success_embed.add_field(
					name=_("Cmd_setnotification_Channel"), value=ch.mention
				)
				# メンションが有効かどうかの項目
				if role:
				...
				success_embed.add_field(
					name=_("Cmd_setnotification_Mention"), value=mention_settings_text
				)
				# 自動削除の項目
				success_embed.add_field(
					...
					value=_("False")
					if auto_delete == 0
					else _("Cmd_setnotification_AutoDelete_Seconds", auto_delete),
				)
	...
	async def viewsettings(self, ctx: discord.ApplicationContext):
		...
		try:
			...
			if status_msg_ch:
				status_msg = await status_msg_ch.fetch_message(
					int(gc.server_status_message.message_id)
				)
			else:
				status_msg = None
			embed.add_field(
